...
task('setAttester', 'Set attester address on verifier')
  ...
  .setAction(async (args: { verifier: string, attester: string }, hre: HardhatRuntimeEnvironment) => {
    ...
    const OPOVPoPVerifier = await ethers.getContractFactory('OPOVPoPVerifier')
    const verifierInstance = OPOVPoPVerifier.attach(args.verifier).connect(signer) as Contract
...
task('setVerifier', 'Set verifier address on attester')
  .addParam('verifier', 'Verifier address')
  .addParam('attester', 'Attester address')
  .setAction(async (args: { verifier: string, attester: string }, hre: HardhatRuntimeEnvironment) => {
    const {ethers} = hre
    ...
    const target = await attesterInstance.getAddress()

    const estimateTx = {
      to: target,
      data: attesterInstance.interface.encodeFunctionData('setVerifier', [args.verifier])
    }
    const gasPrice = await ethers.provider.estimateGas(estimateTx)

    await attesterInstance.setVerifier(args.verifier, {gasPrice})
  })
